...
for (i in 1:12) {
  ...
  write.table(dataframe_echantillon_i[, 1:10], file=sprintf("./3-Fichiers_filtrEs_vcf/%s.vcf", ID[i]), col.names=F,
              quote=F, row.names=F, sep = "\t")
  write.table(dataframe_echantillon_i, file=sprintf("./4-Fichiers_filtrEs_avec_toutes_les_colonnesVCF/%s.vcf", ID[i]),
              col.names=F, quote=F, row.names=F, sep = "\t")
  #######################
  b1_ <- rbind( b1_, dataframe_echantillon_i )
}
b1_ <- b1_[order(b1_$CHROM, as.numeric(b1_$POS)),]
write.table(b1_[, 1:10], file="./3-Fichiers_filtrEs_vcf/batch1-filtrE.vcf", col.names=F,
            quote=F, row.names=F, sep = "\t")
...
write.table(b2_[, 1:10], file="./3-Fichiers_filtrEs_vcf/batch2-filtrE.vcf", col.names=F,
            quote=F, row.names=F, sep = "\t")

b_all <- rbind( b1_, b2_ )
write.table(b_all[, 1:10], file="./3-Fichiers_filtrEs_vcf/echantillon_fusionnE-filtrE.vcf",
            col.names=F, quote=F, row.names=F, sep = "\t")
write.table(b_all, file="./4-Fichiers_filtrEs_avec_toutes_les_colonnesVCF/echantillon_fusionnE-filtrE.vcf",
            col.names=F, quote=F, row.names=F, sep = "\t")
###########################################################################
