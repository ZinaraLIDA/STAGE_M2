...
sample_["Nonsynonymous"] <- unlist( lapply( 1:nrow(sample_), function(i) { nrow( subset( Annotated, (sampleID==sample_$id[i] & impact!="Synonymous") ) ) } ) )
...
write.table( sample_, file="./1-Sortie/sample_.txt", col.names=T, quote=F, row.names=F, sep = "\t")
